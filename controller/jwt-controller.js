...
export const authenticateToken = (request, response, next) => {
    ...
    jwt.verify(token, process.env.ACCESS_SECRETKEY, (error, user) => {
        if (error) {
...
export const createNewToken = async (request, response) => {
    ...
    jwt.verify(token.token, process.env.RANDOM_SECRETKEY, (error, user) => {
        if (error) {
            response.status(500).json({ msg: 'invalid refresh token'});
        }
        const accessToken = jwt.sign(user, process.env.ACCESS_SECRETKEY, { expiresIn: '15m'});
